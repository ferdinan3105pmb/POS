            ...
            <div class="row">
                <div class="col-xxl-12 col-md-12">
                    <div class="card info-card sales-card d-flex flex-row" style="height:60px">
                        ...
                        <a href="{{ route('admin_add_item_type') }}" type="button" style="padding-top:6px;margin-top:10px;margin-right:15px;height:35px;width: 200px" class="px-4 btn btn-sm btn-dark rounded-pill float-right ml-3">
                            Add New</a>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function getData(page) {
        var name = $('#name').val();
        $.ajax({
            url: `/admin/item-type/data?name=` + name + `&page=` + page,
            method: 'GET',
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        ...
                        success: function(res, data) {
                            $('#overlay').css("display", "none");
                            if (res.status == true) {
                                swal("Success", "Staff Berhasil Dihapus!",
                                    "success");
                                getData();
                            } else {
                                toastr['error']('Something Error');
